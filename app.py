...
# ==========================================
    ...
    with t_recurrente:
        st.subheader("Cargar Gastos Fijos")
        df_sus = get_suscripciones()
        if not df_sus.empty:
            
            # --- NUEVA FEATURE: SELECTOR DE FECHA ---
            c_f1, c_f2 = st.columns([1, 2])
            fecha_impacto = c_f1.date_input("Fecha de impacto", date.today().replace(day=5)) 
            c_f2.info("Seleccioná la fecha en la que querés que caigan estos gastos (ej: 5 de Marzo).")
            
            # Tabla editable para ajustar montos antes de cargar
            st.write("Podés ajustar los montos si este mes vinieron distintos:")
            ...
            if st.button(f"🚀 Impactar Fijos para el {fecha_impacto.strftime('%d/%m/%Y')}", type="primary"):
                count = 0
                for i, row in edited_sus.iterrows():
                    # Buscamos los IDs originales usando el índice o descripción
                    ...
                    count += 1
                st.success(f"¡Listo! Se cargaron {count} gastos para {fecha_impacto.strftime('%B')}. Revisá el Dashboard."); time.sleep(2); st.rerun()
        else:
